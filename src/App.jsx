        ...
        <main className="dashboard-main">
          ...
          <section className="app-view-panel">
            <div className="phone-scroll-arrows arrow-prev">
              <button className="arrow-nav-btn" onClick={handlePrevVideo} disabled={currentIndex === 0 || allViewed} title="上滑视频">
                      ...
                      <div className="video-action-sidebar">
                        <div className="sidebar-avatar-wrapper">
                          <div className="sidebar-avatar">
                            <img src={`https://api.dicebear.com/7.x/bottts/svg?seed=${activeVideo.creator_name}`} alt="avatar" />
                          </div>
                        ...
                        {myVideos.length > 0 ? (
                            myVideos.map(mv => (
                                <div key={mv.id} className="grid-video-card">
                                  <img className="grid-video-cover" src={getMediaUrl(mv.cover_url)} alt={mv.title} />
                                  <button className="grid-card-delete-btn" onClick={(e) => handleDeleteVideo(mv.id, e)} title="删除此视频">
                            ...
                            <div className="my-videos-empty">
                              <svg viewBox="0 0 24 24"><path d="M21 3H3c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h18c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-9 9l-6 4.5-6 4.5z" /></svg>
                              <p>您还没有发布任何作品喔</p>
                            </div>
                        )}
                      </div>
                      <div className="my-videos-pagination">
                        <button disabled={myVideosPagination.page === 1} onClick={() => fetchMyVideos(myVideosPagination.page - 1)}>上一页</button>
                        <span>第 {myVideosPagination.page} / {myVideosPagination.totalPages || 1} 页</span>
                        <button disabled={myVideosPagination.page >= myVideosPagination.totalPages} onClick={() => fetchMyVideos(myVideosPagination.page + 1)}>下一页</button>
                      </div>
                    </div>
                )}
              </div>
            ...
            <div className="phone-scroll-arrows arrow-next">
              <button className="arrow-nav-btn" onClick={handleNextVideo} disabled={currentIndex === videos.length - 1 || allViewed} title="下滑视频">
                <svg style={{ width: 20, height: 20, fill: 'currentColor' }} viewBox="0 0 24 24">
          ...
          <section className="dev-console-panel">
            ...
            <div className="terminal-card">
              <div className="terminal-header">
                ...
                <button className="terminal-clear-btn" onClick={() => setDevLogs([])}>Clear</button>
              </div>
                    ...
                    devLogs.map((log, i) => (
                      ...
                      {log.durationMs}ms
                          ...
                          <span
                              ...
                              style={{ cursor: 'pointer', color: 'var(--primary-cyan)', marginLeft: 8, fontSize: 12, userSelect: 'none' }}
                          >
                      详情
                    </span>
                        </div>
              ...
              <div className="modal-content" onClick={e => e.stopPropagation()}>
                ...
                {isUploading ? (
                    <div style={{ textAlign: 'center', padding: '30px 0' }}>
                      ...
                      <p style={{ fontSize: 14, color: 'var(--text-main)', fontWeight: 500 }}>视频上传切片并写入数据库中...</p>
                      <div className="progress-bar-container">
                        <div className="progress-bar-fill" style={{ width: `${uploadProgress}%` }} />
                      </div>
                      <span style={{ fontSize: 12, color: 'var(--text-muted)', display: 'block', marginTop: 8 }}>进度: {uploadProgress}%</span>
                    </div>
                ) : (
                    <form onSubmit={handlePublish}>
                      <div className="upload-form-group">
                        <label>视频标题 *</label>
                        <input type="text" placeholder="输入精彩的视频标题..." value={uploadTitle} onChange={e => setUploadTitle(e.target.value)} required />
                      </div>
                      <div className="upload-form-group">
                        <label>视频描述 (选填)</label>
                        <textarea placeholder="为视频添加补充说明..." value={uploadDesc} onChange={e => setUploadDesc(e.target.value)} />
                      </div>
                      <div className="upload-form-group">
                        <label>选择视频文件 *</label>
                        <div className="file-upload-zone" onClick={() => document.getElementById('video-input-file').click()}>
                          <svg viewBox="0 0 24 24"><path d="M19.35 10.04C18.67 6.59 15.64 4 12 4 9.11 4 6.6 5.64 5.35 8.04 2.34 8.36 0 10.91 0 14c0 3.31 2.69 6 6 6h13c2.76 0 5-2.24 5-5 0-2.64-2.05-4.78-4.65-4.96zM14 13v4h-4v-4H7l5-5 5 5h-3z" /></svg>
                          <p>点击选择 MP4 / WEBM / MOV 格式短视频</p>
                          <span>大小限制 100MB</span>
                          <input id="video-input-file" type="file" accept="video/*" style={{ display: 'none' }} onChange={e => setUploadVideo(e.target.files[0])} required />
                        </div>
                        {uploadVideo && <div className="file-name-indicator">🎥 选定视频: {uploadVideo.name} ({(uploadVideo.size / 1024 / 1024).toFixed(2)} MB)</div>}
                      </div>
                      <div className="upload-form-group">
                        <label>上传封面图片 (选填)</label>
                        <div className="file-upload-zone" onClick={() => document.getElementById('cover-input-file').click()}>
                          <svg viewBox="0 0 24 24"><path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z" /></svg>
                          <p>点击选择封面配图 (JPG / PNG / WEBP)</p>
                          <span>若不传封面，系统将自动生成彩色渐变图占位</span>
                          <input id="cover-input-file" type="file" accept="image/*" style={{ display: 'none' }} onChange={e => setUploadCover(e.target.files[0])} />
                        </div>
                        {uploadCover && <div className="file-name-indicator cover-file">🖼️ 选定封面: {uploadCover.name}</div>}
                      </div>
                      <div className="modal-action-row">
                        <button type="button" className="btn-secondary" onClick={() => setIsUploadOpen(false)}>取消</button>
                        <button type="submit" className="btn-primary">立即发布短视频</button>
                      </div>
                    </form>
                )}
              </div>
              ...
              <div className="modal-content" onClick={e => e.stopPropagation()}
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <pre style={{
                    ...
                    fontSize: 12, color: '#a8ff78', overflowX: 'auto',
                ...
                <div>
                  ...
                  <pre style={{
                    background: 'rgba(0,0,0,0.3)', borderRadius: 6, padding: 12,
                    fontSize: 12, color: '#78c8ff', overflowX: 'auto',
                    whiteSpace: 'pre-wrap', wordBreak: 'break-all',
                    maxHeight: 150, overflowY: 'auto', margin: 0
                  }}>
